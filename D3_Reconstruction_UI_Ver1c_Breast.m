...
% stop watch start
tstart = tic;
axes(handles.axes2)       % display reconstructed image on axes2
title('Calculating ...')

pause(0.01)

Data = handles.data;      % image
Ref = handles.ref;        % reference

% Setup info: Wavelength and sample-to-imager distance
lambda = str2double(get(handles.wavelength,'String'))*1e-9;    % wavelength
Dz = str2double(get(handles.Dz,'String'))*1e-4;                % Dz
...
minx = ceil(xlimit(1));          % boundary of the displayed image
maxx = floor(xlimit(2));         % boundary of the displayed image
miny = ceil(ylimit(1));          % boundary of the displayed image
maxy = floor(ylimit(2));         % boundary of the displayed image
...
if maxy > size(Data,1)
    maxy = size(Data,1);         % make image fit on screen
end
...
subNormAmp = sqrt(Data(miny:maxy,minx:maxx)./(Ref(miny:maxy,minx:maxx))*NormFactor);    % normalize image

        
delta_prev = delta2;

if UpsampleFactor > 0
    [subNormAmp,delta2] = upsampling(subNormAmp,UpsampleFactor,delta_prev);    % make image bigger
end

Nx = size(subNormAmp,1);
Ny = size(subNormAmp,2);      % size of normalized image
delta1 = delta2;
...
%         Threshold_objsupp = 0.040; 
        ...
        Threshold_objsupp = str2double(get(handles.ThresholdObj,'String'));     % object support

        support = stdfilt(abs(Recon1).*cos(angle(Recon1)),ones(9));        % value of each output pixel is the standard deviation of the 9-by-9 neighborhood around the corresponding input pixel.
        support = im2bw(support,Threshold_objsupp);     % replacing all pixels in the input image with luminance greater than level with the value 1 (white) and replacing all other pixels with the value 0
        se = strel('disk',6,0);
...
Phase = angle(ReconImage);      % angle(z) returns the phase angle

Phase_Revise = abs(Phase - mean2(Phase));     % mean2: mean of all values in array
...
if get(handles.radiobuttonColor,'Value')
    ...
    ReconColor(:,:,2) = 1-abs(ReconImage);    % colored reconstructed image
    imagesc(ReconColor)
end
...
% imagesc(handles.data ./ handles.ref)
% imagesc(handles.Submap)
% % plot(handles.Recon)
% axes(handles.axes2)
...
support = handles.ObjSupp;
Phase = handles.Phase;
Phasebackground = mean2(Phase)*3;       % background
Phase = Phase-Phasebackground;
...
fudgefactor = 0.000000001
BW = edge(I, 'roberts', threshold * fudgefactor)
 
se90 = strel('line', 5, 90)
se0 = strel('line', 5, 0)
BWsdil = imdilate(BW, [se90 se0])
 
fill = imfill(BWsdil, 'holes')
% % BWnobord = imclearborder(fill, 4)
% 
SE = strel("disk",7)
BW2 = imerode(fill,SE);
...
% SE = strel('rectangle', [30000 30000])
% BW2 = imopen(BW2, SE)
...
IntCell
sum(IntCell)/size(IntCell,1)    % ans
